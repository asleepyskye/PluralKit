...
- Statistics Database (*InfluxDB*)

*Optionally, it can also integrate with Sentry for error reporting, and InfluxDB for aggregate statistics.

The bot and API are built using .NET 5, with other optional components used for scaling (ex. scheduled_tasks) built using Rust.
# Building + Running
...
```
## Docker (not actively maintained)
Once again, the docker-compose is not actively maintained, however it still does work to run the main bot. This is not recommended. If you are intending to run the bot for development use, please use Nix.
1. Clone the repository: `git clone https://github.com/PluralKit/PluralKit`
2. Create a `pluralkit.conf` configuration file in the `PluralKit` directory
3. Build: `docker-compose build`
4. Run: `docker-compose up` (optionally add the `-d` flag to run detached)
